...
## Local `/conductor` K8s Build

This checkout carries a local Conductor 3.30.2 deployment variant that runs both the engine and ui-next under the `/conductor` context path. Preserve the source-level context path changes; do not replace them with an nginx-only string substitution or ingress rewrite workaround.

Key locations:

- Source root: `conductor-3.30.2`
- Dockerfile: `docker/server/Dockerfile.next`
- Runtime Spring config: `docker/server/config/config-postgres-conductor.properties`
- nginx config: `docker/server/nginx/nginx.conf`
- UI context helper: `ui-next/src/utils/contextPath.ts`
- K8s manifests: `K8s/`
- Full implementation notes: `docs/deployment/conductor-context-path-k8s.md`
